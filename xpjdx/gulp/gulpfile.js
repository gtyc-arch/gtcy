...
function gulp_watch(cb){
    ...
    // 刷新浏览器
    watch(['../*.html', '../css/*.css', '../js/*.js']).on('change', reload)
...
function renameFile(filepath,newname){
    ...
    const dir = config.baseDir == '../' ? config.baseDir + 'img/' : config.baseDir + '/img/'
    const newfilepath = pathHandle(_path.concat(dir, _filename))
    
	fs.rename(filepath,newfilepath,function(err){
		if(err){
			throw err
		}
        console.log(`Starting '${styles.blue[0]}' 路径： ${styles.green[0]}  ${styles.underline[0]}`, 'gulp_images', './img/' + _filename, '可按 Ctrl + V 粘贴')
        getSize(newfilepath)
        exec('clip').stdin.end(encode('./img/' + _filename, 'gbk'));
	})
}
